...
`handoff.md` 由 `feature-discovery` 设置 `Status = Pending Review`，`requirements-reviewer` 审查后更新 `Status` 为 `Ready` / `Ready with Risks` / `Blocked`。只有 `Ready` 或 `Ready with Risks` 才能进入 Loop B。
